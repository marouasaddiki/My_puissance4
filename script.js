    ...
    $.fn.puissance4 = function () {
        class Puissance4 {
            constructor(selector) {
                ...
                $('#go').click(this.grille.bind(this));//bind=lier
            ...
            grille() {
                $('#puissance4').remove();
                $("body").prepend("<div id='puissance4'></div>");// insère le contenu, spécifié par le paramètre, au début de chaque élément dans l'ensemble des éléments correspondants.
                ...
                this.nb_x = $('#x').val();//val obtenir la valeur du premier élèment
                this.nb_y = $('#y').val();
                if (this.nb_x > 3 && this.nb_y > 3 && this.nb_x < 15 && this.nb_y < 15) {
                    for (let row = 0; row < this.nb_x; row++) {
                        ...
                        for (let col = 0; col < this.nb_y; col++) {
                            const $col = $('<div>')
                                .addClass('col empty')
                                .attr('data-col', col)//obtenir la valeur d'un attribut pour le premier élément de l'ensemble d'éléments correspondants.
                                .attr('data-row', row);
                            $row.append($col);
                        }
                        grille.append($row);
            ...
            placerPions() {
                ...
                function recupLastEmpty(col) {
                    ...
                    for (let i = cells.length - 1; i >= 0; i--) {
                        const $cell = $(cells[i]);
                        if ($cell.hasClass('empty')) {//détermine si l'un des éléments correspondants est affecté à la classe donnée.
                            return $cell;
                        }
                    }
                    return null;
                ...
                grille.on('mouseenter', '.col.empty', function () {
                    ...
                    const col = $(this).data('col');//stocke des données arbitraires associées à l'élément spécifié
